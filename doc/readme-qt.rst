...
Alternatively, install Qt Creator and open the `LUNA-qt.pro` file.

An executable named `LUNA-qt` will be built.
...
A warning for people using the *static binary* version of LUNA on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of LUNA is linked against libdb5.3.
...
installed causes LUNA-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
